...
class ResearchAgent:
    ...
    def search_candidates(
        self, queries: list[str], log: StepLogger
    ) -> list[SearchResult]:
        """Run each query on the web and collect unique candidate links."""
        seen: set[str] = set()
        candidates: list[SearchResult] = []
        for q in queries:
            log.wait(f"searching the web for: {q}")
            found = web_search(q)
            new = 0
            for res in found:
                if res.url not in seen:
                    ...
                    new += 1
            log.ok(f"{len(found)} result(s), {new} new link(s)")
        log.info(
    ...
    def read_sources(
        self, candidates: list[SearchResult], log: StepLogger
    ) -> list[Source]:
        ...
        for res in candidates:
            if len(sources) >= config.MAX_SOURCES:
                break
            log.wait(f"opening: {res.url}")
            text = fetch_text(res.url)
            if text:
                sources.append(Source(title=res.title, url=res.url, text=text))
                log.ok(f"read {len(text)} chars  <- {res.title[:50]}")
            else:
                log.skip(f"could not read (skipped): {res.url}")
        return sources

    def synthesize(
        self, topic: str, sources: list[Source], log: StepLogger, trace: RunTrace
    ) -> str:
        """Combine sources into a cited report."""
        if not sources:
            log.skip("no readable sources -> cannot write a report")
            return (
        ...
        log.wait("model is writing the report (this is the slow part)...")
        ...
        report, tokens = self.llm.chat_with_tokens(
            system=SYNTHESIS_SYSTEM, user=user, temperature=0.3
        )
        trace.add_tokens(**tokens)
        return report
    ...
    def research(
        ...
        # All per-run state (logger, trace) is local, so one shared (singleton)
        # agent can serve concurrent requests without them clobbering each other.
        log = StepLogger(enabled=verbose, sink=log_sink)
        trace = RunTrace(topic, self.llm.model)
        log.start(topic)

        log.step(1, TOTAL_STEPS, "PLAN", "turn your topic into search queries")
        with trace.step("plan"):
            queries = self.plan(topic, log, trace)
        trace.metric("num_queries", len(queries))
        log.step_done(f"{len(queries)} query(ies) ready")

        log.step(2, TOTAL_STEPS, "SEARCH", "look those queries up on the web")
        with trace.step("search"):
            candidates = self.search_candidates(queries, log)
        trace.metric("candidate_links", len(candidates))
        log.step_done(f"{len(candidates)} candidate link(s) found")

        log.step(3, TOTAL_STEPS, "READ", "open pages and extract their text")
        with trace.step("read"):
            sources = self.read_sources(candidates, log)
        trace.metric("sources_read", len(sources))
        log.step_done(f"{len(sources)} readable source(s) collected")

        log.step(4, TOTAL_STEPS, "WRITE", "synthesize a cited report")
        with trace.step("write"):
            report = self.synthesize(topic, sources, log, trace)
        trace.metric("report_chars", len(report))
        log.step_done(f"report is {len(report)} chars")
